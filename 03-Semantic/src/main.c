...
static void print_help(FILE *out, const char *prog) {
  ...
  fprintf(out, "\nOBS\n");
  fprintf(out,
          "    Sem opções, apenas os diagnósticos semânticos são exibidos.\n");
}
 ...
 */
static OutputFlags parse_output_flags(int argc, char *argv[],
                                      const char **filename) {
...
/**
 * @brief Imprime a tabela de erros sintacticos que impediram a analise
 * semantica.
 */
static void print_syntax_errors(const Parser *parser) {
  printf("=== ERROS SINTÁTICOS ===\n");
...
int main(int argc, char *argv[]) {
  ...
  if (parser.error_count > 0) {
    ...
    struct timespec sem_start, sem_end;
    ...
    printf("Análise concluída em %.1f ms.\n", elapsed_ms(sem_start, sem_end));
