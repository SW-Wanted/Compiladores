 ...
 */
static void fill_parameters(SemAnalyzer *a, ASTNode *param_list,
                            SemSymbol *sym) {
  sym->param_count = 0;
  if (!param_list)
    return;
  for (int i = 0; i < param_list->child_count; i++) {
    ...
    SemType t = (p->child_count > 1)
                    ? sem_type_from_declarator(base, p->children[1])
                    : base;
...
static void register_function(SemAnalyzer *a, ASTNode *func) {
  ...
  if (existing) {
    ...
    if (existing->is_defined && has_body) {
      sem_error(&a->diags, symbol.line, symbol.column,
                "Redefinição da função '%s' (definida na linha %d).", name,
                existing->line);
      return;
    }
...
static void register_define(SemAnalyzer *a, ASTNode *node) {
  ...
  if (node->child_count > 1 && node->children[1]->text) {
    ...
    if (v[0] == '"') {
      type = sem_type_make(TOKEN_CHAR, NULL);
      type.pointer_level = 1;
    } else if (strchr(v, '.'))
      type = sem_type_make(TOKEN_DOUBLE, NULL);
  }
...
static void analyze_function(SemAnalyzer *a, ASTNode *func) {
  ...
  ASTNode *param_list = func->children[2];
  ASTNode *body =
      (func->child_count > 3 && func->children[3]->kind == AST_BLOCK)
          ? func->children[3]
          : NULL;
  if (!body)
    return;
...
static void collect_globals(SemAnalyzer *a, ASTNode *list) {
  for (int i = 0; i < list->child_count; i++) {
    ...
    switch (decl->kind) {
    case AST_FUNC_DECL:
      register_function(a, decl);
      break;
    case AST_TYPEDEF_DECL:
      register_typedef(a, decl);
      break;
    case AST_DIRECTIVE_DEFINE:
      register_define(a, decl);
      break;
...
static void check_globals(SemAnalyzer *a, ASTNode *list) {
  for (int i = 0; i < list->child_count; i++) {
    ...
    switch (decl->kind) {
    case AST_FUNC_DECL:
      analyze_function(a, decl);
      break;
    case AST_VAR_DECL:
      sem_check_var_decl(a, decl, 1);
      break;
    default:
      break;
    }
...
/**
 * @brief Executa a analise semantica sobre a arvore e devolve o numero de
 * erros.
 */
