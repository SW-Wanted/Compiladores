...
/**
 * @brief Verdadeiro se o no e o literal inteiro zero (constante de ponteiro
 * nulo).
 */
...
typedef enum { COMPAT_OK, COMPAT_NARROW, COMPAT_PTR, COMPAT_BAD } CompatResult;

/**
 * @brief Verdadeiro se o tipo e um identificador nao resolvido (tipo externo
 * opaco).
 */
 ...
 */
static CompatResult type_compatible(SemAnalyzer *a, SemType dst, SemType src,
                                    ASTNode *src_node) {
 ...
 */
void sem_check_assign_compat(SemAnalyzer *a, SemType dst, SemType src,
                             ASTNode *src_node, int line, int column,
                             const char *context) {
  ...
  case COMPAT_PTR:
    sem_warning(&a->diags, line, column,
                "%s: tipos de ponteiro incompatíveis ('%s' e '%s').", context,
                ssrc, sdst);
    break;
  case COMPAT_BAD:
    sem_error(&a->diags, line, column,
              "%s: incompatibilidade de tipos (não é possível usar '%s' onde "
              "se espera '%s').",
              context, ssrc, sdst);
    break;
  default:
...
/**
 * @brief Resultado das conversoes aritmeticas usuais (o tipo de maior rank, >=
 * int).
 */
...
/**
 * @brief Verifica o uso de um identificador, reportando se nao estiver
 * declarado.
 */
...
static SemType check_assignment(SemAnalyzer *a, ASTNode *node, const char *op) {
  ...
  if (!sem_is_lvalue(left)) {
    sem_error(&a->diags, sem_line(node), sem_col(node),
              "O lado esquerdo de '%s' não é um lvalue (não pode receber "
              "atribuição).",
              op);
    return lt;
  }
  ...
  if (strcmp(op, "=") == 0) {
    sem_check_assign_compat(a, lt, rt, right, sem_line(node), sem_col(node),
                            "Atribuição");
  } else {
    ...
    if (bitwise) {
      if (lt.valid && rt.valid &&
          (!sem_type_is_integer(&lt) || !sem_type_is_integer(&rt)))
        sem_error(&a->diags, sem_line(node), sem_col(node),
                  "operador '%s' requer operandos inteiros", op);
    } else if (lt.valid && rt.valid &&
               (!sem_type_is_scalar(&lt) || !sem_type_is_scalar(&rt))) {
      sem_error(&a->diags, sem_line(node), sem_col(node),
...
static SemType check_binary(SemAnalyzer *a, ASTNode *node) {
  ...
  if (is_relational_op(op)) {
    if (sem_type_is_aggregate(&lt) || sem_type_is_aggregate(&rt))
      sem_error(
          &a->diags, sem_line(node), sem_col(node),
          "O operador '%s' não pode comparar tipos agregados (struct/union).",
          op);
    else if (sem_type_is_pointer(&lt) != sem_type_is_pointer(&rt) &&
             !is_zero_literal(node->children[0]) &&
             !is_zero_literal(node->children[1]))
  ...
  if (ptr_l || ptr_r) {
    if ((op_is(op, "+") || op_is(op, "-")) &&
        ((ptr_l && sem_type_is_integer(&rt)) ||
         (sem_type_is_integer(&lt) && ptr_r)))
      return ptr_l ? lt : rt;
    if (op_is(op, "-") && ptr_l && ptr_r)
      return sem_type_int();
    sem_error(&a->diags, sem_line(node), sem_col(node),
              "O operador '%s' é inválido para operandos do tipo ponteiro.",
              op);
    return sem_type_invalid();
  }
...
/**
 * @brief Verifica uma expressao unaria (prefixa ou pos-fixa) e devolve o seu
 * tipo.
 */
...
/**
 * @brief Verifica uma chamada de funcao: existencia, aridade e tipos dos
 * argumentos.
 *
...
static SemType check_call(SemAnalyzer *a, ASTNode *node) {
  ...
  if (!fn) {
    sem_warning(&a->diags, sem_line(callee), sem_col(callee),
                "Função '%s' não declarada (assumida como externa).",
                callee->text);
    for (int i = 0; i < argc; i++)
      sem_check_expr(a, args->children[i]);
    return sem_type_int();
  ...
  if (!fn->is_variadic && argc != fn->param_count) {
    sem_error(&a->diags, sem_line(callee), sem_col(callee),
              "Função '%s' espera %d argumento(s), mas recebeu %d.", fn->name,
              fn->param_count, argc);
  } else if (!fn->is_variadic) {
    for (int i = 0; i < argc && i < fn->param_count && i < SEM_MAX_PARAMS;
         i++) {
      ...
      if (line < 0) {
        line = sem_line(callee);
        col = sem_col(callee);
      }
      sem_check_assign_compat(a, fn->param_types[i], arg_types[i],
                              args->children[i], line, col, ctx);
    }
  }
...
static SemType check_subscript(SemAnalyzer *a, ASTNode *node) {
  ...
  if (arr.valid && !sem_type_is_pointer(&arr) &&
    ...
    sem_error(&a->diags, sem_line(node->children[0]),
              sem_col(node->children[0]),
              "Indexação ('[]') aplicada a tipo não indexável '%s'.", s);
    return sem_type_invalid();
  }
  if (idx.valid && !sem_type_is_integer(&idx))
    sem_error(&a->diags, sem_line(node->children[1]),
              sem_col(node->children[1]),
              "O índice de um array deve ser um inteiro.");

  if (arr.array_level > 0)
    arr.array_level--;
  else if (arr.pointer_level > 0)
    arr.pointer_level--;
  return arr;
}
...
static SemType check_member(SemAnalyzer *a, ASTNode *node) {
  ...
  if (arrow) {
    if (obj.pointer_level != 1 ||
        (obj.base != TOKEN_STRUCT && obj.base != TOKEN_UNION)) {
      if (!is_opaque(&obj))
        sem_error(&a->diags, sem_line(node), sem_col(node),
...
/**
 * @brief Verifica uma expressao qualquer, devolvendo o seu tipo e emitindo
 * diagnosticos.
 */
SemType sem_check_expr(SemAnalyzer *a, ASTNode *node) {
  ...
  switch (node->kind) {
  case AST_IDENTIFIER:
    return check_identifier(a, node);
  case AST_LITERAL:
    return infer_literal(node->text);
  case AST_BINARY_EXPR:
    return check_binary(a, node);
  case AST_UNARY_EXPR:
    return check_unary(a, node);
  case AST_CALL_EXPR:
    return check_call(a, node);
  case AST_SUBSCRIPT_EXPR:
    return check_subscript(a, node);
  case AST_MEMBER_EXPR:
    return check_member(a, node);
  case AST_CONDITIONAL_EXPR:
    return check_conditional(a, node);
  case AST_ERROR:
    return sem_type_invalid();
  default:
