 ...
 */
static SemSymbol *declare_variable(SemAnalyzer *a, SemType base,
                                   ASTNode *declarator) {
  ...
  if (duplicate && slot)
    sem_error(&a->diags, line, column,
              "Redeclaração de '%s' no mesmo escopo (declaração anterior na "
              "linha %d).",
              name, slot->line);
  return slot;
}
 ...
 */
static void check_initializer(SemAnalyzer *a, SemSymbol *sym, SemType type,
                              ASTNode *init) {
...
void sem_check_var_decl(SemAnalyzer *a, ASTNode *node, int is_global) {
  ...
  SemType first_type =
      first ? first->type : sem_type_from_declarator(base, first_decl);

  int idx = 2;

  if (idx < node->child_count &&
      node->children[idx]->kind != AST_DECLARATOR_LIST) {
  ...
  if (idx < node->child_count &&
    ...
    for (int i = 0; i < more->child_count; i++) {
      ...
      SemType dtype =
          sym ? sym->type : sem_type_from_declarator(base, declarator);
      if (i + 1 < more->child_count &&
          more->children[i + 1]->kind != AST_DECLARATOR) {
 ...
 */
static void check_condition(SemAnalyzer *a, ASTNode *expr,
                            const char *keyword) {
  if (!expr)
    return;
  ...
  if (t.valid && !sem_type_is_scalar(&t)) {
    ...
    sem_error(&a->diags, sem_line(expr), sem_col(expr),
              "A condição de '%s' deve ser de tipo escalar (obtido '%s').",
              keyword, s);
  }
}
